...
if [[ $REBUILD_NPM_MODULES ]]; then
  echo "=> myktra/meteord:bin-build is NOT TESTED AT ALL (and maybe not necessary???)"
  ...
  echo "     discourage use of the :bin-build image if possible!  Thanks! -myktra"
  if [ -f /opt/meteord/rebuild_npm_modules.sh ]; then
    cd programs/server
    bash /opt/meteord/rebuild_npm_modules.sh
    cd ../../
  else
    echo "=> Use myktra/meteord:bin-build for binary bulding."
    exit 1
  fi
fi
...
node --max_old_space_size=512 --gc_interval=100 main.js
